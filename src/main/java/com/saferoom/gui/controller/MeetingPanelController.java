...
public class MeetingPanelController {
    ...
    // FXML Değişkenleri
    @FXML
    private AnchorPane centerContentAnchorPane;
    @FXML
    private StackPane contentStackPane;
    @FXML
    private VBox rightPanelVBox;
    @FXML
    private ToggleButton participantsToggle;
    @FXML
    private ToggleButton chatToggle;
    @FXML
    private ScrollPane participantsScrollPane;
    @FXML
    private StackPane meetingChatView;
    @FXML
    private ChatViewController meetingChatViewController;
    @FXML
    private GridPane videoGrid;
    @FXML
    private BorderPane speakerViewPane;
    @FXML
    private Label meetingNameLabel;
    @FXML
    private Label timerLabel;
    @FXML
    private JFXButton participantsButtonTop;
    @FXML
    private JFXButton gridViewButton;
    @FXML
    private JFXButton speakerViewButton;
    @FXML
    private VBox participantsListVBox;
    @FXML
    private JFXButton micButton;
    @FXML
    private JFXButton cameraButton;
    @FXML
    private JFXButton chatButtonBottom;
    @FXML
    private JFXButton screenShareButton;
    @FXML
    ...
    private final Map<String, VideoTrack> remoteVideoTracks = new java.util.concurrent.ConcurrentHashMap<>(); // username
                                                                                                              // →
                                                                                                              // VideoTrack
    ...
    public void initData(Meeting meeting, UserRole userRole, boolean withCamera, boolean withMic, String joinMode) {
        ...
        // ESKİ TASARIM: Keep participants list (real peers will be added/removed
        // dynamically)
        currentMeeting.getParticipants().clear();
        currentMeeting.getParticipants().add(0, this.currentUser);

        updateParticipantsList();
        showGridView(); // ESKİ TASARIM: Immediately show grid (will show placeholder until video
                        // arrives)
    ...
    private void setupGroupCallCallbacks() {
        ...
        groupCallManager.setOnPeerJoinedCallback(peerUsername -> {
            javafx.application.Platform.runLater(() -> {
                ...
                // ESKİ TASARIM: Add peer to participant list (camera OFF initially, will turn
                // ON when track arrives)
    ...
    private Node createParticipantListItem(Participant participant) {
        ...
        if (!participant.isCameraOn())
            camIcon.getStyleClass().add("icon-danger");

        itemBox.getChildren().addAll(avatar, nameBox, spacer, micIcon, camIcon);

        List<MenuItem> menuItems = roleStrategy.createParticipantMenuItems(currentUser, participant,
        ...
        itemBox.setOnMouseClicked(event -> {
            if (event.getButton() == MouseButton.SECONDARY) {
                if (contextMenu.isShowing()) {
                    ...
                    // Bu metot, JavaFX'in pencere sınırlarını otomatik olarak algılaması için en
                    // iyisidir.
    ...
    private void updateSpeakerView() {
        ...
        if (currentMeeting.getParticipants().isEmpty())
            return;
     ...
     * Her participant için tile oluşturur (camera on/off durumuna göre video veya
     * avatar)
     */
